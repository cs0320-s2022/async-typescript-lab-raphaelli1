...
const suggestionElements: HTMLElement = document.getElementById("suggestions") as HTMLElement;
...
function updateSuggestions(matches: string[]): void {
  ...
  for (let i = 0; i < matches.length; i ++) {
    suggestionElements.innerHTML += `<li tabindex=\"${i}\">${matches[i]}</li>\n`
  }
}
...
document.addEventListener("keyup", function (event) {
  if (event.key == 'w') {
    updateValues("Leo", "Capricorn", "Cancer").then(postAndUpdate);
  }
});
